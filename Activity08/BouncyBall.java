...
public class BouncyBall extends JPanel
{
	...
	public void paintComponent(Graphics g)
	{
		...
		if (x + xDelta < radius) // off left
			xDelta = -xDelta; // flip direction
		else if (x + xDelta > width - radius) // off right
			xDelta = -xDelta; // flip direction
		x += xDelta; // increment
		
		// TODO: Add conditional statements to keep ball in bounds
		//       in the y axis (e.g. top and bottom edges)
		if (y + yDelta < radius) // off top
			yDelta = -yDelta; // flip direction
		else if (y + yDelta > height - radius) // off bottom
			yDelta = -yDelta;
		y += yDelta; // increment
		
		// Check if off screen
		if (x > width - radius)
			x = Math.max(width - radius, 0); // handle width < radius
		else if (x < 0)
			x = 0;
		if (y > height - radius)
			y = Math.max(height - radius, 0); // handle height < radius
		else if (y < 0)
			y = 0;
	...
	public BouncyBall()
	{
		...
		int rndR = random.nextInt(255);
		int rndG = random.nextInt(255);
		int rndB = random.nextInt(255);
		color = new Color(rndR,rndG,rndB);
		...
		x = random.nextInt(INIT_WIDTH - radius);
		y = random.nextInt(INIT_HEIGHT - radius);
