...
# Description: Grader for JukeboxHeroEC project
...
echo "---------------------------------------"
echo "Compiling JukeboxHeroEC.java using javac"
echo "---------------------------------------"

javac JukeboxHeroEC.java
if [ ! $? == 0 ] # Make sure the program built
then
  echo "FAIL reason: JukeboxHeroEC.java did not compile"
  exit 1
else
  echo "JukeboxHeroEC.java compiled successfully"
fi

echo "--------------------------"
echo "Beginning (L)oad test"
echo "--------------------------"
java JukeboxHeroEC <<END | awk '{ print "\t" $0 }'
L
