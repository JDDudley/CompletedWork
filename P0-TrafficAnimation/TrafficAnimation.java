 ...
 *
 * Animated ocean scene with interacting elements.
 *
...
public class TrafficAnimation extends JPanel {
	...
	public void paintComponent(Graphics g) {
		...
		if (!hooked) { // Keep moving everything with offsets
			...
			Font hookedFont = new Font(g.getFont().getFontName(), g.getFont().getStyle(),
					g.getFont().getSize() * width / 200);
			g.setFont(hookedFont);
			g.setColor(Color.black);
			g.drawString("FISH ON!", width / 3, height / 4);
		...
		// Clouds
		int cloud = width / 48;
		int cloudX = width * 3 / 4;
		int cloudY = height / 4;
		g.setColor(Color.white);
		// Generate cloud set
		for (int x = 2; x < 5; x++) {
			int cloudSize = cloud * x;
			g.fillOval(cloudX + cloudSize, cloudY - cloudSize, cloudSize, cloudSize);		
		}
		// Water colors
		...
		if (baitX - fishX > -8 && baitX - fishX < 8 && 
			...
			hooked = true;
		// Draw fishing line if fish behind
		if (fishY < baitY) {
			g.setColor(Color.white);
			g.drawLine(hullX - anglerHeight / 3, hullY - cabinHeight * 2, hullX - cabinLength, height * 3 / 4);
		}
